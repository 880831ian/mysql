...
if [ "$?" = "1" ];then
mysqladmin -uroot -pqweqwe create dbei
else
echo -e "${a}dbei 已存在${b}"
fi
if [ "$?" = "0" ];then
...
var=`expr $var + 1`;num=`expr $num + 1`;
mysql -uroot -pqweqwe -e 'use dbei;show columns from exim' >/dev/null 2>&1
if [ "$?" = "1" ];then
mysql -uroot -pqweqwe -e 'use dbei;create table exim(id int, title varchar(40) not null)'
else
...
var=`expr $var + 1`;num=`expr $num + 1`;
mysql -uroot -pqweqwe -e 'use dbei;select *from exim' | grep -o "eximx" >/dev/null 2>&1
if [ "$?" = "1" ];then
mysql -uroot -pqweqwe -e "use dbei;insert into exim(title)values('eximx')"
else
...
var=`expr $var + 1`;num=`expr $num + 1`;
mysql -uroot -pqweqwe -e 'use dbei;select *from exim' | grep -o "eximy" >/dev/null 2>&1
if [ "$?" = "1" ];then
mysql -uroot -pqweqwe -e "use dbei;insert into exim(title)values('eximy')"
else
...
mysql -uroot -pqweqwe -e "use dbei;select * from exim into outfile  '/tmp/exim-0.txt' fields terminated by '<>' enclosed by '!' lines terminated by '\r\n'"
fi
if [ "$?" = "0" ];then
...
mysqldump -uroot -pqweqwe dbei exim > /tmp/dbeieximdump.txt
fi
if [ "$?" = "0" ];then
...
mysqldump -uroot -pqweqwe dbei > /tmp/dbeidump.sql
fi
if [ "$?" = "0" ];then
...
var=`expr $var + 1`;num=`expr $num + 1`;
mysqldump -uroot -pqweqwe --tab=/tmp dbei exim
if [ "$?" = "0" ];then
...
mysqldump -uroot -pqweqwe dbei exim > /tmp/dbeieximdump.sql
fi
if [ "$?" = "0" ];then
...
mysqldump -uroot -pqweqwe dbei  > /tmp/dbeidump1.sql
fi
if [ "$?" = "0" ];then
